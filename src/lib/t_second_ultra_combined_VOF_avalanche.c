...
int DDT_density_velocity_second_ultra_combined_VOF_avalanche(in *I, int p, int i, int j, int k)
{
	if (check_for_corrupt_cell(I, i, j, k)) return 1;
	double A_value;
	A_value = density(I, i, j, k) / (2 * I->dt);
	...
	I->B[A_IND(I, p, i, j, k)] += density(I, i, j, k) * (
		...
		velocity(I, p, i, j, k + 1)) / (6. * 2 * I->dt);
	A_value = velocity(I, p, i, j, k) / (2 * I->dt);
	WRITE_TO_A(3, i, j, k, -1);
	I->B[A_IND(I, 3, i, j, k)] += velocity(I, p, i, j, k) * (
		density(I, i - 1, j, k) +
		density(I, i + 1, j, k) +
		density(I, i, j - 1, k) +
		density(I, i, j + 1, k) +
		density(I, i, j, k - 1) +
		density(I, i, j, k + 1)) / (6. * 2 * I->dt);
	return 0;
}

int DDT_density_second_ultra_combined_VOF_avalanche(in *I, int p, int i, int j, int k)
{
	if (check_for_corrupt_cell(I, i, j, k)) return 1;
	double A_value;
	A_value = 1 / I->dt;
...
int DDT_pressure_cont_second_ultra_combined_VOF_avalanche(in *I, int p, int i, int j, int k)
{
	if (check_for_corrupt_cell(I, i, j, k)) return 1;
