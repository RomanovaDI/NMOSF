		...
		for (int i = 0; i < I->nx; i++) {
			for (int j = 0; j < I->ny; j++) {
				if (I->ind_cell_multipl[i * I->ny + j] != -1) {
					...
					I->B_prev[B_IND(I, 5, i, j, k)] = I->residual_saturation[0] + I->epsilon + I->residual_saturation[0];
					I->B_prev[B_IND(I, 6, i, j, k)] = 1 - 2 * I->residual_saturation[0] - I->residual_saturation[2] - 2 * I->epsilon;
